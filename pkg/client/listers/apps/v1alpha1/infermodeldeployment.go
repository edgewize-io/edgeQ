...
type InferModelDeploymentLister interface {
	...
	List(selector labels.Selector) (ret []*appsv1alpha1.InferModelDeployment, err error)
...
type inferModelDeploymentLister struct {
	listers.ResourceIndexer[*appsv1alpha1.InferModelDeployment]
}
...
func NewInferModelDeploymentLister(indexer cache.Indexer) InferModelDeploymentLister {
	return &inferModelDeploymentLister{listers.New[*appsv1alpha1.InferModelDeployment](indexer, appsv1alpha1.Resource("infermodeldeployment"))}
}

// InferModelDeployments returns an object that can list and get InferModelDeployments.
func (s *inferModelDeploymentLister) InferModelDeployments(namespace string) InferModelDeploymentNamespaceLister {
	return inferModelDeploymentNamespaceLister{listers.NewNamespaced[*appsv1alpha1.InferModelDeployment](s.ResourceIndexer, namespace)}
}
...
type InferModelDeploymentNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*appsv1alpha1.InferModelDeployment, error)
