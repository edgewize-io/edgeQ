...
Usage: $(basename "$0") <generators> <output-package> <apis-package> <groups-versions> ...
  ...
  <output-dir>        the output package name (e.g. github.com/example/project/pkg/generated).
...
if [ "${GENS}" = "all" ] || grep -qw "client" <<<"${GENS}"; then
  ...
  "${GOPATH}"/bin/client-gen --clientset-name "${CLIENTSET_NAME_VERSIONED:-versioned}" --input-base "" --input "$(codegen::join , "${FQ_APIS[@]}")" --output-dir "${OUTPUT_PKG}"/"${CLIENTSET_PKG_NAME:-clientset}" "$@"
fi

if [ "${GENS}" = "all" ] || grep -qw "lister" <<<"${GENS}"; then
  echo "Generating listers for ${GROUPS_WITH_VERSIONS} at ${OUTPUT_PKG}/listers"
  "${GOPATH}"/bin/lister-gen --input-dirs "$(codegen::join , "${FQ_APIS[@]}")" --output-dir "${OUTPUT_PKG}"/listers "$@"
fi
