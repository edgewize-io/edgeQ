...
def read_images():
    ...
    images_buffer = []
    image_files = []
    abs_images_path = os.environ.get("ABS_IMAGES_PATH")
    if not abs_images_path:
        abs_images_path = "/opt/mindspore/serving/example/resnet/test_image"

    for path, _, file_list in os.walk(abs_images_path):
...
def run_classify_top1(first=False):
    ...
    client = Client(target_host, "resnet50", "classify_top1")
    instances = []
