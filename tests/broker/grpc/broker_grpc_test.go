...
import (
	...
	bg "github.com/edgewize/edgeQ/internal/broker/endpoint/grpc"
...
var _ = Describe("双向流代理测试", func() {
	var (
		brokerServer  *bg.GRPCBrokerServer
	...
	BeforeEach(func() {
		...
		brokerConf.Broker.Type = constants.GRPCEndpoint
		brokerConf.ServiceGroups = []brokerCfg.ServiceGroup{
			{
				Name:        "default",
				Reclaimable: true,
				Weight:      10,
		...
		brokerConf.Broker.GRPC.BackendAddr = lis.Addr().String()

		brokerServer, err = bg.NewGRPCBrokerServer(brokerConf)
		if err != nil {
			Fail(fmt.Sprintf("创建 proxy 服务器失败 %v", err))
		}

		go brokerServer.Start(context.Background())

		conn, err := grpc.Dial(
			fmt.Sprintf("127.0.0.1:%s", constants.DefaultBrokerContainerPort),
